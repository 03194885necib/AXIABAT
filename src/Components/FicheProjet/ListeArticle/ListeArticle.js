...
// No longer need Firestore imports for this component
// import { collection, doc, updateDoc } from 'firebase/firestore';

// Accept projectTVA as a prop
function ListeArticle({ listeArt, onArticleUpdated, projectTVA }) {
    const [tableData, setTableData] = useState([]);
    ...
    useEffect(() => {
        // Inform the parent component of changes in articles
        // The articles passed back will no longer have an individual tvaRate
        if (onArticleUpdated && tableData.length > 0) {
            onArticleUpdated(tableData);
        }
    }, [tableData, onArticleUpdated]);
    ...
    // Use projectTVA for calculation
    const calculateTVAAmount = (item) => {
        const totalPrice = parseFloat(calculateTotalPrice(item));
        // Use the project's TVA rate (ensure it's a number and converted to a decimal)
        const projectTVARate = parseFloat(projectTVA) / 100 || 0;
        return (totalPrice * projectTVARate).toFixed(2);
    };
            ...
            <table className="invoice-table">
                <thead>
                    <tr>
                        {[
                            'N° Article', 'Désignation', 'Qté', 'Unité',
                            'Prix Unitaire', 'TVA Projet (%)', 'Prix HT', // Changed header
                            'Montant TVA', 'Prix TTC'
                        ].map(header => <th key={header}>{header}</th>)}
                    </tr>
                </thead>
                <tbody>
                    {tableData.map((item) => (
                        <tr key={item.id}>
                            ...
                            {/* Display Project TVA (read-only) instead of individual TVA input */}
                            <td>
                                {parseFloat(projectTVA).toFixed(2) || '0.00'}%
                            </td>
                ...
                <tfoot>
                    {[
                        ...
                        <tr key={label}>
                            <td colSpan="5" style={{ textAlign: 'right', fontWeight: 'bold' }}>{label}</td> {/* Adjusted colSpan */}
                            {/* Empty cell for TVA Projet column in footer */}
